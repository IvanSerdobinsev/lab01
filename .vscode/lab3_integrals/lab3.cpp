...
// === Точное разбиение Veltkamp-Dekker ===
void Split(float a, float& a_high, float& a_low) {
    const float factor = 4097.0f;
    float c = factor * a;
    a_high = c - (c - a);
    a_low = a - a_high;
}
...
float integrate_with_fma(float dv, float v_max, float T) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Одна операция: |v| * pdf + sum с одним округлением
...
int main() {
    ...
    cout << setprecision(8);
    cout << "  T   |   Аналит |     Naive |     Kahan  |     my_fma |     Double     |   ε_naive |      ε_kahan |      ε_fma   |      ε_double\n";
